...
完整架构说明见 [`docs/architecture/`](docs/architecture/)。

## 不可破坏的安全不变式 / Invariants (do not break)

- admin（install/uninstall、模块配置）仅 ROOT 可达。
- 非 ROOT 路径逐笔强制 `to != address(this) && to != address(0)`。
...
完整不变式与威胁分析见 [`docs/security/`](docs/security/)。
...
- MegaETH 的 `0x100` 预编译未被官方确认，依赖 passkey 前先上链探测（见 [`docs/deployment/megaeth-testnet.md`](docs/deployment/megaeth-testnet.md)）。
