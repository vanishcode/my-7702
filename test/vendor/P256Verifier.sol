...
// NOTE: TEST-ONLY VENDORED DEPENDENCY. Not used by `src/`.
// 仅用于测试：在本地 EVM(prague) 不带 0x100 预编译时，用 vm.etch 将此合约置于 0x100。
// Test-only: under local EVM (prague) which lacks the 0x100 precompile, etch this at 0x100.
// Source (verbatim, pragma relaxed to ^0.8.21):
//   https://raw.githubusercontent.com/daimo-eth/p256-verifier/master/src/P256Verifier.sol
